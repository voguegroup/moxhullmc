...
if ($_POST['submitted']) {
	...
	if (!$erun) {
		...
		$query = sprintf("INSERT INTO products (range_id,
												product_name, 
												product_description,
												product_colour,
												product_gradient,
		...
		mysql_real_escape_string($product_gradient),
		...
		if ($result) {
			$erun = 'Product Created';
			$srun = "Product &quot;" . mysql_real_escape_string(stripslashes($_POST['product_name'])) . "&quot; Created";
			include($cms_root_url . '/components/log-script.php');
		} 
		else {
			$erun = 'Error, Product NOT Created';
		}
	}
...
<form action="<?php echo $GP_uploadAction; ?>" method="post" enctype="multipart/form-data" name="addform" onsubmit="checkFileUpload(this,'JPG,JPEG',true,'','','','','','','');showProgressWindow('fileCopyProgress.htm',300,100);return document.MM_returnValue">
...
<select name="range_id" size="1">
	...
	<?php
		...
		while($row = mysql_fetch_assoc($result)) {
			...
			if ($_POST['range_id'] == $row['range_id']) {
...
<input name="product_name" type="text" size="80" maxlength="100" value="<?php echo stripslashes($_POST['product_name']); ?>" />
<img src="../images/icon-required-field.gif" alt="This is a required field and *MUST* be completed" name="required-field-icon" width="18" height="18" hspace="5" vspace="0" border="0" style="vertical-align:top;" /><br /><br />

Enter a Product Description<br />
<textarea name="product_description" rows="5" cols="40"><?php echo stripslashes($_POST['product_description']); ?></textarea><br /><br />
...
<input name="product_gradient" type="text" size="1" maxlength="2" value="<?php echo stripslashes($_POST['product_gradient']); ?>" /><br /><br />

Is This a Patterned Carpet?<br />
<input type="radio" name="product_patterned" value="Y" <?php if ($_POST['product_patterned'] == 'Y') { echo 'checked="checked"'; } ?> />Yes<br />
<input type="radio" name="product_patterned" value="N" <?php if (!isset($_POST['product_patterned']) || empty($_POST['product_patterned']) || $_POST['product_patterned'] == 'N') { echo 'checked="checked"'; } ?> />No<br /><br />

Is This Product a Bestseller?<br />
<input type="radio" name="product_bestseller" value="Y" <?php if ($_POST['product_bestseller'] == 'Y') { echo 'checked="checked"'; } ?> />Yes<br />
<input type="radio" name="product_bestseller" value="N" <?php if (!isset($_POST['product_bestseller']) || empty($_POST['product_bestseller']) || $_POST['product_bestseller'] == 'N') { echo 'checked="checked"'; } ?> />No<br /><br />

Status<br />
<select name="product_status" size="1">
	<option value="L" <?php if ($_POST['product_status'] == 'L') { echo 'selected="selected"'; } ?>>Online</option>
    <option value="D" <?php if ($_POST['product_status'] == 'D') { echo 'selected="selected"'; } ?>>Offline</option>
</select><br /><br />
