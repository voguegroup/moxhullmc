...
if ($_POST['submitted']) {
	...
	if (!$erun) {
		$query = sprintf("UPDATE rooms SET room_name='%s', room_image='%s' WHERE room_id='%s'",
		mysql_real_escape_string(stripslashes($_POST['room_name'])),
		mysql_real_escape_string(stripslashes($_POST['room_image'])),
		mysql_real_escape_string($room_id));
	
		$result = mysql_query($query);
		
		if ($result) {
			$erun = 'Room Saved';
			$srun = "Room &quot;" . mysql_real_escape_string(stripslashes($_POST['room_name'])) . "&quot; Edited";
			include($cms_root_url . '/components/log-script.php');
		} 
		else {
	...
	if ($_GET['delete_image']) {
		...
		$query = "UPDATE rooms SET room_image='' WHERE room_id=" . mysql_real_escape_string($room_id);
		
		$result = mysql_query($query);
			
		if ($result) {
			$erun = 'Room Image Deleted';
		} 
		else {
			$erun = 'Error, Room Image NOT Deleted';
		}	
...
$query = "SELECT * FROM rooms WHERE room_id=" . mysql_real_escape_string($room_id);
		...
		<div id="page-container">
			<div id="page-pad">
                ...
                <div id="page-pad-title">
                    ...
                    <p>Modify an existing product type.</p>
                </div>
...
<form action="<?php echo $GP_uploadAction; ?>" method="post" enctype="multipart/form-data" name="editform" onsubmit="checkFileUpload(this,'JPG,JPEG',true,'','30','30','30','30','','');showProgressWindow('fileCopyProgress.htm',300,100);return document.MM_returnValue">
	
Enter a Product Type Name<br />
<input type="text" name="room_name" size="80" maxlength="200" value="<?php echo stripslashes($room_name); ?>" />
...
<img src="../images/icon-required-field.gif" alt="This is a required field and *MUST* be completed" name="required-field-icon" width="18" height="18" hspace="5" vspace="0" border="0" style="vertical-align:top;" /><br /><br />
<?php } else { ?>
Current Icon Image<br />
<img src="/images/rooms/<?php echo $room_image; ?>" alt="" /><br />
<input type="hidden" name="room_image" value="<?php echo $room_image; ?>" />
<a href="rooms-edit.php?edit=<?php echo $room_id; ?>&amp;delete_image=true">Delete current image and upload a new one</a><br /><br />
<?php } ?>

<input name="submitted" type="hidden" value="TRUE" />
<input name="submit" type="submit" value="Save Product Type" />
