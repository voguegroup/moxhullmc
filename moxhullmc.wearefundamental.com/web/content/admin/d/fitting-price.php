...
if ($_POST['submitted']) {
	...
	$query = sprintf("UPDATE fitting SET fitting_price='%s' WHERE fitting_id=1",
	mysql_real_escape_string(stripslashes($_POST['fitting_price'])));
	
	$result = mysql_query($query);
	
	if ($result) {
		$erun = 'Fitting Price Updated';
		$srun = $erun;
		include($cms_root_url . '/components/log-script.php');
	}
...
$query = "SELECT * FROM fitting WHERE fitting_id=1";
$result = mysql_query($query);
$row = mysql_fetch_assoc($result);
$fitting_price = $row['fitting_price'];
		...
		<div id="page-container">
			<div id="page-pad">
                ...
                <div id="page-pad-title">
                    <h1>Fitting Price</h1>
                    <p>Alter the standard fitting price.</p>
                </div>
                
                <form action="" method="post">
                
                	Enter a new fitting price:<br />
                	&pound;<input type="text" name="fitting_price" size="5" maxlength="5" value="<?php echo $fitting_price; ?>" /><br /><br /><br />
               		<input type="hidden" name="submitted" value="TRUE" />
                	<input type="submit" name="submit" value="Update Fitting Price" />
