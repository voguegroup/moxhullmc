...
if ($_POST['submitted']) {
	...
	// Validation	
	if (!$erun && empty($_POST['manufacturer_name'])) {
		$erun = "Please enter a manufacturer name";
	}
	
	
	if (!$erun) {
		$query = sprintf("UPDATE manufacturers SET manufacturer_name='%s' WHERE manufacturer_id='%s'",
		mysql_real_escape_string(stripslashes($_POST['manufacturer_name'])),
		mysql_real_escape_string($manufacturer_id));
	
		$result = mysql_query($query);
		
		if ($result) {
			$erun = 'Manufacturer Saved';
			$srun = "Manufacturer &quot;" . mysql_real_escape_string(stripslashes($_POST['manufacturer_name'])) . "&quot; Edited";
			include($cms_root_url . '/components/log-script.php');
		} 
		else {
			$erun = 'Error, Manufacturer NOT Saved';
		}
	}
...
$query = "SELECT * FROM manufacturers WHERE manufacturer_id=" . mysql_real_escape_string($manufacturer_id);
		...
		<div id="page-container">
			<div id="page-pad">
                ...
                <div id="page-pad-title">
                    ...
                    <p>Modify an existing manufacturer.</p>
                </div>
...
Enter a Manufacturer Name<br />
<input type="text" name="manufacturer_name" size="80" maxlength="200" value="<?php echo stripslashes($manufacturer_name); ?>" />
...
<input name="submit" type="submit" value="Save Manufacturer" />
