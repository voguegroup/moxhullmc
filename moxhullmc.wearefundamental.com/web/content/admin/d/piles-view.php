...
if ($_POST['submitted']) {
	...
	$query = sprintf("INSERT INTO piles (pile_name) VALUES ('%s')",
	mysql_real_escape_string(stripslashes($_POST['pile_name'])));
	
	$result = mysql_query($query);
	
	if ($result) {
		$srun = 'Pile "' . mysql_real_escape_string(stripslashes($_POST['pile_name'])) . '" Added';
		include($cms_root_url . '/components/log-script.php');
	}
	...
	if ($_GET['delete']) {
		...
		$query = "DELETE FROM piles WHERE pile_id=" . mysql_real_escape_string($_GET['delete']);
...
function deleteAlert(id) {
	document.getElementById('delete_alert').innerHTML = '<div id="error_message_bg"></div><div id="error_message"><p>Do you really want to delete this pile?<br /><br /><input type="button" value="Yes" onclick="deleteYes('+id+')" />&nbsp;&nbsp;&nbsp;<input type="button" value="No" onclick="deleteNo()" /></p></div>';
}

function deleteYes(id) {
	location.href='piles-view.php?delete='+id;
}
		...
		<div id="page-container">
			<div id="page-pad">
                ...
                <div id="page-pad-title">
                    <h1>Suitability</h1>
                    <p>Add and remove suitability.</p>
                </div>
                
                <form action="" method="post">
                
                Create a new suitability:<br />
                <input type="text" name="pile_name" size="30" maxlength="200" />
                <input type="hidden" name="submitted" value="TRUE" />
                <input type="submit" name="submit" value="Create a suitability" />
					...
					if (mysql_num_rows($query_result) > 0) {
						...
						while ($row = mysql_fetch_assoc($query_result)) {
							...
							echo '<tr onmouseover="this.style.backgroundColor=\'#f4f4f4\';" onmouseout="this.style.backgroundColor=\'#fafafa\';">';
								...
								echo '<td align=\"left\" valign=\"middle\"><a href="piles-edit.php?edit=' . $row['pile_id'] . '" class="button">Edit</a></td>';
								echo '<td align=\"left\" valign=\"middle\"><a onclick="deleteAlert(' . $row['pile_id'] . ')" class="button" >Delete</a></td>';
							echo '</tr>';
